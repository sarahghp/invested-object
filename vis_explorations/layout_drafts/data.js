...
var conx = (function(cx, moments, tests){
...
})(conxList, populated_moments, tests);

// Make elevation/distance intersection
var elevations = _.filter(conxList, {type: 'Elevation'});
var distances = _.filter(conxList, {type: 'Distance From Home'});
var mix = _.map(elevations, (e) => {
  return _.map(distances, (d) => {
    var unionized = _.intersectionBy(e.members, d.members, 'title')
    return { 
      mix: e.modifier + '-' + d.modifier, 
      members: unionized
    }
  })
});

// Make temp-weather-humidity intersections
var temps = _.filter(conxList, {type: 'Temp'});
var hums = _.filter(conxList, {type: 'Humidity'});
var weathers = _.filter(conxList, {type: 'Weather'});

var threeMix = _.map(temps, (t) => {
  return _.flatten(_.map(hums, (h) => {
    return _.map(weathers, (w) => {
      var intersected =  _.intersectionBy(t.members, h.members, w.members, 'title');
      return {
        mix: t.modifier + '-' + h.modifier + '-' + w.modifier,
        members: intersected
      }
    })
  }))
